/**
 * Email templates for FaithReach notifications.
 * Each function returns { subject, html, text } for the email.
 */
...
export function ticketResolvedEmail(data: {
  ...
  const body = `
    ...
    <p style="margin:0 0 20px;color:#52525b;">Hi ${data.userName},</p>
    <p style="margin:0 0 16px;color:#374151;">
      Great news! Your support ticket <strong>"${data.ticketSubject}"</strong> has been resolved.
    </p>
    ${summaryBlock}
    <table width="100%" cellpadding="0" cellspacing="0" style="margin:20px 0;">
      <tr>
        <td style="padding:8px 0;color:#71717a;font-size:14px;">Ticket ID</td>
        <td style="padding:8px 0;color:#18181b;font-size:14px;text-align:right;font-family:monospace;">${data.ticketId.slice(0, 8)}</td>
      </tr>
      <tr>
        <td style="padding:8px 0;color:#71717a;font-size:14px;">Resolved At</td>
        <td style="padding:8px 0;color:#18181b;font-size:14px;text-align:right;">${data.resolvedAt}</td>
      </tr>
    </table>
...
export function ticketStatusUpdateEmail(data: {
  ...
  const label = data.newStatus.replace(/_/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase());

  const body = `
    <h2 style="margin:0 0 8px;font-size:20px;color:#18181b;">Ticket Status Updated</h2>
    <p style="margin:0 0 20px;color:#52525b;">Hi ${data.userName},</p>
    <p style="margin:0 0 16px;color:#374151;">
      Your support ticket <strong>"${data.ticketSubject}"</strong> has been updated.
    </p>
    ...
    <p style="margin:20px 0 0;color:#52525b;font-size:14px;">
      Ticket ID: <code style="background:#f4f4f5;padding:2px 6px;border-radius:4px;">${data.ticketId.slice(0, 8)}</code>
    </p>
    <p style="margin:16px 0 0;color:#52525b;font-size:14px;">
      Blessings,<br/><strong>The FaithReach Support Team</strong>
    </p>
...
Your support ticket "${data.ticketSubject}" status has changed to: ${label}
